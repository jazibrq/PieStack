...
export const BulletHellGame = ({ onGameOver, onExit }: BulletHellGameProps) => {
  ...
  const update = useCallback((dt: number) => {
    ...
    if (s.boss) {
      ...
      for (let i = s.playerBullets.length - 1; i >= 0; i--) {
        ...
        if (dist(b, boss) < boss.size + b.size) {
          ...
          if (boss.hp <= 0) {
            ...
            s.waveTimer = 0;
            s.spawnTimer = 0;
            s.spawnRate = Math.max(443, Math.floor(1415 * Math.pow(0.82, s.stage - 1)));
            s.enemyBullets = [];
            s.enemies = [];
            s.score += SCORE_STAGE;
      ...
      const waveSpawnRate = Math.max(319, s.spawnRate * Math.pow(0.90, s.wave - 1));
      ...
      if (s.waveTimer >= WAVE_DURATION) {
        ...
        s.spawnRate = Math.max(354, Math.floor(1415 * Math.pow(0.82, s.stage - 1) * (1 - waveProgress * 0.4)));
